...
async function setupWizard(): Promise<SetupAnswers> {
  ...
  if (needGemini) {
    ...
    // Ask for model preference
    console.log(chalk.yellow("\nChoose Gemini model:"));
    console.log("  1. gemini-2.5-flash (Recommended - Fast and cheap)");
    ...
    answers.geminiModel =
      geminiModelChoice === "2" ? "gemini-1.5-pro" : "gemini-2.5-flash";
  }
  ...
  if (!needGemini) {
    answers.geminiModel = "gemini-2.5-flash";
  }
...
function displaySummary(answers: SetupAnswers): void {
  ...
  console.log(chalk.gray("     docker run -p 8000:8000 chromadb/chroma"));
  console.log(chalk.yellow("\n  2. Index a repository:"));
  console.log(chalk.gray("     code-rag index /path/to/your/project"));
  console.log(chalk.yellow("\n  3. Ask questions:"));
  console.log(chalk.gray('     code-rag ask "how does authentication work?"'));
  console.log();
}
